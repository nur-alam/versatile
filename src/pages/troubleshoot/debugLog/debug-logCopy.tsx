import { useSearchParams } from 'react-router-dom';
import { ServerDataTableCopy, Column, TFetchDataPromise, ActionHandlers, ActionButton } from '@/pages/troubleshoot/debugLog/data-tableCopy';
import { Eye, Edit, Trash2, Download, Share, Copy } from 'lucide-react';

export type DebugRow = {
	key?: React.Key;
	id: number;
	name: string;
	email: string;
	role: string;
	createdAt: string;
};

export type DebugLogSearchParams = {
	page?: number;
	pageSize?: number;
	search?: string;
	sortKey?: string;
	order?: string;
};

const debugLogCopy = () => {
	// Use React Router's useSearchParams for hash-based routing
	const [searchParams] = useSearchParams();

	const columns = [
		{ key: "id", header: "ID", sortable: false },
		{ key: "name", header: "Name", sortable: true },
		{ key: "email", header: "Email", sortable: true },
		{ key: "role", header: "Role", sortable: true },
		{
			key: "createdAt", header: "Created", sortable: true, render: (v?: string, row?: DebugRow) => {
				console.log('createdAt', v, row);
				return new Date(v || '').toLocaleDateString()
			},
		},
		{ key: "actions", header: "Actions", sortable: false },
	] as Column<DebugRow>[];

	// New flexible action buttons approach
	const actionButtons: ActionButton<DebugRow>[] = [
		{
			key: 'view',
			icon: Eye,
			onClick: (row) => {
				console.log('View:', row);
				alert(`Viewing user: ${row.name}`);
			},
			title: 'View',
			ariaLabel: 'View item',
			colorScheme: 'blue'
		},
		{
			key: 'edit',
			icon: Edit,
			onClick: (row) => {
				console.log('Edit:', row);
				alert(`Editing user: ${row.name}`);
			},
			title: 'Edit',
			ariaLabel: 'Edit item',
			colorScheme: 'green'
		},
		{
			key: 'download',
			icon: Download,
			onClick: (row) => {
				console.log('Download:', row);
				alert(`Downloading data for: ${row.name}`);
			},
			title: 'Download',
			ariaLabel: 'Download item data',
			colorScheme: 'purple'
		},
		{
			key: 'share',
			icon: Share,
			onClick: (row) => {
				console.log('Share:', row);
				alert(`Sharing: ${row.name}`);
			},
			title: 'Share',
			ariaLabel: 'Share item',
			colorScheme: 'yellow'
		},
		{
			key: 'delete',
			icon: Trash2,
			onClick: (row) => {
				console.log('Delete:', row);
				if (confirm(`Are you sure you want to delete ${row.name}?`)) {
					alert(`Deleted user: ${row.name}`);
				}
			},
			title: 'Delete',
			ariaLabel: 'Delete item',
			colorScheme: 'red'
		},
		{
			key: 'copy',
			icon: Copy,
			onClick: (row) => {
				console.log('Copy:', row);
				if (confirm(`Are you sure you want to copy ${row.name}?`)) {
					alert(`Copied user: ${row.name}`);
				}
			},
			title: 'Copy',
			ariaLabel: 'Copy item',
			colorScheme: 'blue'
		},
	];

	// Legacy action handlers (kept for backward compatibility)
	const actionHandlers: ActionHandlers<DebugRow> = {
		onView: (row) => {
			console.log('View:', row);
			alert(`Viewing user: ${row.name}`);
		},
		onEdit: (row) => {
			console.log('Edit:', row);
			alert(`Editing user: ${row.name}`);
		},
		onDelete: (row) => {
			console.log('Delete:', row);
			if (confirm(`Are you sure you want to delete ${row.name}?`)) {
				alert(`Deleted user: ${row.name}`);
			}
		},
	};

	// Dummy API simulation
	function fetchData({ page, pageSize, search, sortKey, order }: DebugLogSearchParams): Promise<{ data: DebugRow[], total: number, totalPages: number }> {
		console.log('fetchData', { page, pageSize, search, sortKey, order });
		return new Promise((resolve) => {
			setTimeout(() => {
				let all = Array.from({ length: 30 }).map((_, i) => ({
					id: i + 1,
					name: `User ${i + 1}`,
					email: `user${i + 1}@example.com`,
					role: ["Admin", "Editor", "Viewer"][i % 3],
					createdAt: new Date(2025, i % 12, (i % 28) + 1).toISOString() || '',
				}));

				// search
				if (search) {
					all = all.filter((u) =>
						u.name.toLowerCase().includes(search.toLowerCase()) ||
						u.email.toLowerCase().includes(search.toLowerCase())
					);
				}

				// sort
				if (sortKey && order) {
					all.sort((a, b) => {
						const av = a[sortKey as keyof typeof a] || '';
						const bv = b[sortKey as keyof typeof b] || '';
						const cmp = String(av).localeCompare(String(bv));
						return order === "asc" ? cmp : -cmp;
					});
				}

				// pagination
				const total = all.length;
				const start = ((page || 1) - 1) * (pageSize || 10);
				const data = all.slice(start, start + (pageSize || 10));
				const totalPages = Math.ceil(total / (pageSize || 10));

				resolve({ data: data as DebugRow[], total, totalPages });
			}, 500);
		});
	}

	return (
		<div className="mx-auto max-w-6xl p-6">
			<h1 className="mb-4 text-2xl font-bold tracking-tight">Users</h1>
			<p className="mb-6 text-slate-600">Server-side pagination, search, and sorting with Tailwind + React.</p>
			<ServerDataTableCopy<DebugRow, TFetchDataPromise<DebugRow>, typeof searchParams>
				columns={columns}
				fetchData={fetchData}
				searchParams={searchParams}
				actionButtons={actionButtons}
				// actionHandlers={actionHandlers} // Use this for legacy approach
			/>
		</div>
	);
}

export default debugLogCopy; 
