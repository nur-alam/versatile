...
import { useGetMoodInfo, useUpdateMaintenanceMood } from '@/services/mood-services';
		...
		<div className="">
			{isLoading || !isFormInitialized ? (
				...
				<ErrorBoundary
					...
					fallback={
						<div className="vt-p-6 vt-border vt-border-red-200 vt-bg-red-50 vt-rounded-md">
							<h3 className="vt-text-lg vt-font-medium vt-text-red-700 vt-mb-2">
								{__('Form Error', 'versatile-toolkit')}
							</h3>
							<p className="vt-text-red-600">
								{__(
									'There was a problem loading the maintenance settings form. Please try refreshing the page or contact support.',
									'versatile-toolkit',
								)}
							</p>
							<Button className="vt-mt-4" onClick={() => window.location.reload()}>
				...
				>
					<Form {...maintenanceMoodForm}>
						...
						>
							<div className="vt-flex vt-justify-between vt-pb-5">
								...
								<div className="vt-flex vt-gap-5">
									<Button type="submit" disabled={updateMaintenanceMoodMutation.isPending}>
							...
							<div>
								<FormField
									...
									render={({ field, fieldState }) => (
										<FormItem className="vt-mt-6">
											<FormLabel className="vt-text-foreground">
												{__('Choose Template', 'versatile-toolkit')}
											</FormLabel>
											<FormControl>
												<ErrorBoundary
													fallback={
														<div className="vt-p-4 vt-border vt-border-red-200 vt-bg-red-50 vt-rounded-md">
															<p className="vt-text-red-700 vt-text-sm">
																{__(
																	'Template selector failed to load. Please try refreshing the page.',
																	'versatile-toolkit',
																)}
															</p>
														</div>
											...
											{!fieldState.error && (
												<FormDescription>
													{__(
														'Select a design template for your maintenance page.',
														'versatile-toolkit',
													)}
												</FormDescription>
											)}
							...
							<div className="vt-flex vt-gap-10 vt-mt-5">
								<div className="vt-w-1/2">
									<FormField
										control={maintenanceMoodForm.control}
										...
										render={({ field, fieldState }) => (
											<FormItem>
												<div className="vt-flex vt-items-center vt-gap-2">
													<FormLabel
														className="vt-text-foreground"
														htmlFor="enable_maintenance"
													>
												...
												{!fieldState.error && (
													<FormDescription>
														{__(
															'Enable or disable maintenance mode for your site.',
															'versatile-toolkit',
														)}
													</FormDescription>
												)}
										...
										render={({ field, fieldState }) => (
											<FormItem className="vt-mt-6">
												<div className="vt-flex vt-items-center vt-gap-2">
													<FormLabel
														className="vt-text-foreground"
														htmlFor="show_subscribers_only"
													>
														{__('Show Only for Users', 'versatile-toolkit')}
													</FormLabel>
													<FormControl>
														<Switch
															id="show_subscribers_only"
															checked={field.value}
															onCheckedChange={field.onChange}
														/>
													</FormControl>
												...
												{!fieldState.error && (
													<FormDescription>
														{__(
															'When enabled, maintenance mode will only be shown to subscribers. Other users will see the normal site.',
															'versatile-toolkit',
														)}
													</FormDescription>
												)}
									...
									<FormField
										...
										render={({ field, fieldState }) => (
											<FormItem className="vt-mt-6">
												...
												<FormControl>
													<Input
														placeholder={__('Enter maintenance title', 'versatile-toolkit')}
														{...field}
													/>
												</FormControl>
												{!fieldState.error && (
									...
									<FormField
										...
										render={({ field, fieldState }) => (
											<FormItem className="vt-mt-6">
												<FormLabel className="vt-text-foreground">
													{__('Subtitle', 'versatile-toolkit')}
												</FormLabel>
												<FormControl>
													<Input
														placeholder={__('Enter subtitle', 'versatile-toolkit')}
														{...field}
													/>
												</FormControl>
												{!fieldState.error && (
									...
									<FormField
										...
										render={({ field, fieldState }) => (
											<FormItem className="vt-mt-6">
												<FormLabel className="vt-text-foreground">
													{__('Description', 'versatile-toolkit')}
												</FormLabel>
												<FormControl>
													<Textarea
														placeholder={__(
												...
												{!fieldState.error && (
													<FormDescription>
														{__(
															'Provide more details about the maintenance.',
															'versatile-toolkit',
														)}
													</FormDescription>
												)}
								...
								<div className="vt-w-1/2">
									<FormField
										...
										render={({ field, fieldState }) => (
											<FormItem className="">
												<FormLabel className="vt-text-foreground">
													{__('Background Image', 'versatile-toolkit')}
												</FormLabel>
												<FormControl>
													<ErrorBoundary
														fallback={
															<div className="vt-p-4 vt-border vt-border-red-200 vt-bg-red-50 vt-rounded-md">
																<p className="vt-text-red-700 vt-text-sm">
																	{__(
																		'Media uploader failed to load. Please try refreshing the page.',
																		'versatile-toolkit',
																	)}
																</p>
															</div>
												...
												{!fieldState.error && (
													<FormDescription>
														{__(
															'Upload a background image for the maintenance page.',
															'versatile-toolkit',
														)}
													</FormDescription>
												)}
										...
										render={({ field, fieldState }) => (
											<FormItem className="vt-mt-6">
												...
												{!fieldState.error && (
													<FormDescription>
														{__(
															'Upload a logo to display on the maintenance page.',
															'versatile-toolkit',
														)}
													</FormDescription>
												)}
