...
function versatile_verify_request( $check_auth = true ) {
	...
	if ( $check_auth && ! is_user_logged_in() ) {
		return array(
			'success' => false,
			'message' => __( 'Access denied! Please login to access this feature.', 'verstaile-toolkit' ),
			'code'    => 403,
		);
	}
	...
	if ( $check_auth && ( ! isset( $_REQUEST[ $nonce_key ] )
		...
		return array(
			'success' => false,
			'message' => __( 'Invalid security token!', 'verstaile-toolkit' ),
			'code'    => 400,
		);
	}
	...
	return array(
		'success' => true,
		'message' => __( 'Verification successful.', 'verstaile-toolkit' ),
		'code'    => 200,
